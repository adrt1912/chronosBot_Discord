...
public class ComandoRank implements Comando {
    ...
    public void ejecutar(ContextoComando ctx) {
        ...
        Member objetivo = event.getOption("usuario") != null ? Objects.requireNonNull(event.getOption("usuario")).getAsMember() : event.getMember();

        if (objetivo == null) ctx.responder("❌ **Error** | No se ha podido encontrar a ese usuario en el servidor.");
        else {
            if (objetivo.getUser().isBot())
                ctx.responder("🤖 Los bots operamos más allá del sistema de experiencia mortal.");
            else {
                ...
                porcentaje = Math.min(100, Math.max(0, porcentaje)); // Asegura que esté entre 0 y 100
    ...
    public SlashCommandData getDatosComando() {
        return Commands.slash("rank", "Muestra tu nivel actual, experiencia y progreso en el servidor.")
                .addOption(OptionType.USER, "usuario", "El usuario del que quieres consultar el rango.", false);
    }
}