...
import aperez.economia.comandos.*;
import aperez.experiencia.comandos.ComandoRank;
import aperez.experiencia.comandos.ComandoTopXP;
import aperez.notificaciones.comandos.*;
import aperez.utilidad.comandos.ComandoAyuda;
import aperez.utilidad.comandos.ComandoClean;
import aperez.utilidad.comandos.ComandoWarn;
...
public class LectorDeComandos extends ListenerAdapter {
    ...
    private void actualizarTarjetaEnVivo (ButtonInteractionEvent event,int idTarea) {
        ...
        if (tarea != null) {
            ...
            } else if (tarea.botonesTipo() == 2) {
                nuevoEmbed.setColor(new Color(0x3498db));
                String[] opciones = tarea.opciones().split("\\|");
    ...
    public void despacharSlash(String nombreComando, SlashCommandInteractionEvent event) {
        ...
        if (comando != null) {
            ...
            comando.ejecutar(new ContextoComando(event));
