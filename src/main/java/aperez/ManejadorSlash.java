...
import aperez.notificaciones.comandos.NotificacionesBD;
import aperez.notificaciones.comandos.Tarea;
    ...
    public void onCommandAutoCompleteInteraction(CommandAutoCompleteInteractionEvent event) {
        ...
        if (event.getFocusedOption().getName().equals("id")) {
            ...
            List<Command.Choice> opcionesMenu = tareas.stream()
                    .filter(t -> String.valueOf(t.id()).contains(loQueEstaEscribiendo)
                            || t.titulo().toLowerCase().contains(loQueEstaEscribiendo))
                    .map(t -> {
                        // Creamos el texto visual que verá el usuario en el menú desplegable de Discord
                        String textoVisual = "📌 [ID: " + t.id() + "] " + t.titulo();
                        ...
                        return new Command.Choice(textoVisual, t.id());
                    })
